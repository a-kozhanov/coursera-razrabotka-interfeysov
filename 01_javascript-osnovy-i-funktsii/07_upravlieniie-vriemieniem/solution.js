/*
Управление временем. Решение
В реализации будем использовать объект Date для хранения текущего значения.

Это позволит не заботиться о корректности даты после манипуляций.

Все сеттеры объекта Date умеют обрабатывать переполнения конкретных единиц и делать правильный пересчет даты.

То есть, вызвав setMinutes(500), получим корректную дату, которая отличается от исходной на 500 минут.
*/
